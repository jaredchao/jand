...
- **对话**：协议仍在开发，Relay 升级时查看 README 的「兼容性」一节。0.4.3 的 Relay 兼容 0.4.0 到 0.4.2 的客户端（0.4.3 起默认有效期 30 分钟，新增 `/r` 接收链接说明页）；0.3.x 与 0.4.x 之间未验证。升级对话协议时，提前通知使用对话的同事一起升级客户端。
