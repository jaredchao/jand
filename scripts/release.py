...
def agent_doc(windows, relay):
    ...
    text = re.sub(r"^<!-- (mode:\w+|/mode|release-only|/release-only|topic:\w+) -->\n", "", text, flags=re.MULTILINE)
...
也可以不自己动手，把这段话发给你的 Agent（Relay 地址换成实际的）：「请在 <这个目录> 运行 `{exe} setup --yes --relay <Relay 地址> --agents claude --allow-claude`，如果它说需要访问令牌先问我要，完成后运行 `{exe} version` 给我看结果。」

## 2. 使用：直接跟你的 Agent 说

- **交出去**：「用 jand 把这个任务交接给 XX」。Agent 会给你一条接收链接，你用微信、钉钉等发给对方。链接默认 30 分钟内有效，只能用一次；对方还没装 jand 也没关系，点开链接就能看到怎么装。
- **收进来**：把对方给的链接交给你的 Agent：「用 jand 收一下：<链接>」。Agent 会先把内容和对方的请求讲给你听，你同意了它才动手。
...
- 双方必须用同一个 Relay。
- Relay 只在内存里暂存密文，过了有效期没人领取就作废；单个文件最大 10 MiB。
- 出了问题：`{exe} version` 看版本和 Relay 是否兼容，`{exe} config` 看当前配置，或者重新运行 `{exe} setup`，它最后的自检会告诉你卡在哪一步。
- 不用了：`{exe} uninstall`。它会先列出要删、要改、要留的东西，默认先把对话历史打包给你，再清理干净；收到的文件默认保留。
