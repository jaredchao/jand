...
jand 让不同机器上的 AI Agent 安全地交接任务，并在需要时远程协作。当前版本 **0.4.3**。
...
jand chat log <chat>            # 终端里的时间线；--brief 每条一行，--follow 边看边更新，对话结束时自动退出
...
`jand help agent` 打印给 Agent 的操作契约（即 [AGENT.md](docs/AGENT.md)），并填好本机程序的实际路径和配置的 Relay。为了省 token，默认只打印核心部分（速查和全部安全规则，约为全文的四分之一），其余按需读取：`jand help agent send|receive|chat|host`，`all` 打印全文；收到对话邀请等时刻，jand 的输出会带 `guide` 字段提示该读哪一节。`jand help template` 打印交接文件的结构。任何能执行命令的 Agent 都能用：在它的全局指令里加一句「使用 jand 前先运行 `jand help agent` 并照做」即可。默认推荐 Claude Code。
...
- **交接协议**自 0.2 起没有变过：0.2.x 客户端可以继续通过 0.4.3 的 Relay 收发，新旧客户端互相收发也可以。
  - 为了兼容，线协议仍沿用 `handoff/0.2`、`/v1/handoffs/` 和 `X-Handoff-*`，这些不是当前的程序名。
  - 旧版接收端遇到以 `-` 开头的接收码时，在码前加 `--`。0.3 起生成的码不再以 `-` 开头。
- **对话协议**仍在开发，版本之间不保证兼容。
  - 0.4.3 与 0.4.1、0.4.2 互通：对话协议自 0.4.1 起没变（已用真实 0.4.1 程序在 0.4.3 Relay 上双向验证交接与对话）。0.4.3 之前的客户端不认接收链接，要改给接收码和 Relay 地址。
  - 0.4.2 与 0.4.1 互通：0.4.2 只在本地输出上多了 Relay 地址。
