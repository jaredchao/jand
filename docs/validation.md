...
实测：`help agent` 全文约 14,000 字符（中文约 6,300 字），约 8–9k token，每个会话读一遍；Ubuntu 那次对话的完整 `chat log` 6,915 字符。

- 说明书分层：AGENT.md 按 `topic:core|send|receive|chat|host` 重排。`help agent` 默认只打印 core（速查、向用户汇报、全部安全规则、退出码、硬约束、章节目录），`help agent <主题>` 按需读取，`all` 为全文；发行包仍是全文。去掉 core 中重复的表述（「这是什么」与速查、前提与速查、速查里的退出码行、「不要做」里与安全契约重复的三条并入契约），安全规则一条未删。core 3,891 字符（全文 14,751），少约 72%；只做接收的会话读 core + receive 约 4,700 字符。测试钉住：core 不超过全文三分之一，且包含安全契约、硬约束、不要做、对话安全约束、退出码与读 chat 的提示；各主题不含安全契约、不留标记、都是全文的一部分。
- 按需提示：`saved`（对话邀请）、`queued`（发起对话）、`joined` 事件带 `"guide":"chat"`，Agent 在进入对话时才读对话规则。
